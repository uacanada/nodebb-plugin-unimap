...
Plugin.addPlaceFieldsToTopic = async (hookData) => {
  const { topicData, uid, privileges, loggedInUser } = hookData;
  const settings = await getSettings({ forceRefresh: false }); 
  let modifiedTopicData = topicModifier(topicData, settings, uid, privileges, loggedInUser);
  return modifiedTopicData; 
}
