...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const isAuthorized = (data.privileges.uid && Number(topic.uid) === Number(data.privileges.uid)) || data.privileges.isAdminOrMod;
      const editButtonHtml = isAuthorized ? `<a href="#!" data-place-author="${topic.uid}" data-topic="${m.tid}" class="btn circle-button edit-place card-link"><i class="fa-regular fa-pen-to-square"></i></a>` : '';
