...
module.exports = (data, settings, uid, privileges, loggedInUser) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const editButtonHtml = isAuthorized ? `<a href="/${settings.mapPageRouter}/edit/${m.tid}" data-place-author="${m.uid}" data-topic="${m.tid}" class="btn circle-button edit-place card-link p-4"><i class="fa-regular fa-pen-to-square"></i></a>` : ``;
