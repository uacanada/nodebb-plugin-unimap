"use strict";
module.exports = (data, settings, uid, privileges, loggedInUser) => {
  try {
    const { topic } = data;
    ...
    if (mapFields?.placeTitle) {
      ...
      const isAuthorized = Number(m.uid) === Number(data.loggedInUser?.uid)|| data.privileges?.isAdminOrMod;
      const editButtonHtml = isAuthorized ? `<a href="/${settings.mapPageRouter}/edit/${m.tid}" data-place-author="${m.uid}" data-topic="${m.tid}" class="btn circle-button edit-place card-link p-4"><i class="fa-regular fa-pen-to-square"></i></a>` : `${uid, privileges, loggedInUser}`;
