...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const isAuthorized = Number(m.uid) === Number(caller.uid); // TODO add admin rights checking
      const editButtonHtml = isAuthorized ? `<a href="#!" data-place-author="${m.uid}" data-topic="${m.tid}" class="btn circle-button edit-place card-link p-4"><i class="fa-regular fa-pen-to-square"></i></a>` : ``;
          ...
          ? `<div id="topicEvent">
              <div class="calendar-item">
                ...
                <div class="calendar-details p-3">
                  <h3 class="calendar-title"><i class="fa-regular fa-calendar-check"></i> ${ m.eventName || m.eventTitle ||  m.fullname || m.placeTitle || ""}</h3>
                  ${eventTime}
                  ${eventWeekDay}
                  ${eventCategory}
                  ${eventLocation}
      ...
      <div id="metaTab" class="tab-content" data-authorcolor="${authorColor}" data-authoravatar="${authorAvatar}" data-authorname="${author.displayname || author.username}" data-authorletter="${authorLetter}">
        ...
        <div class="place-ctx-buttons p-3">
          <div class="place-ctx-buttons-wrapper d-flex align-items-center">
          ${editButtonHtml}
          ...
          <a href="/topic/${m.tid}"  title="Comment this" type="button" class="ms-2 btn circle-button go-to-comment p-4 text-bg-primary"><i class="fa-regular fa-comment-dots"></i></a>
          </div>
        </div>
        `;
    ...
    data.unimap_debug = data.privileges?.isAdminOrMod ? error.message : 'Backend Error...' 
  }
