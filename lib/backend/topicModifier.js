...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const editLink = app.user?.uid && (app.user.isAdmin || app.user.isGlobalMod || Number(topic.uid) === Number(app.user.uid))? `<a href="#!" data-place-author="${topic.uid}" data-topic="${m.tid}" class="btn circle-button edit-place card-link"><i class="fa-regular fa-pen-to-square"></i></a>`: "";
