...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const isAuthorized = (data.privileges?.uid && m.uid && Number(m.uid) === Number(data.privileges.uid)) || data.privileges?.isAdminOrMod;
    ...
    data.unimap_debug = error.message // TODO delete!
  }
