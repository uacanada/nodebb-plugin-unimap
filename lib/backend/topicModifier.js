...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const m = mapFields;
      if(!m.uid) {
        data.topic.posts[0].content += `No place uid`
        return data;
      }
      ...
      const isAuthorized = (data.loggedInUser?.uid && Number(m.uid) === Number(data.loggedInUser?.uid)) || data.privileges?.isAdminOrMod;
