...
module.exports = (data, settings) => {
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const isAuthorized = (data.privileges.uid && Number(m.uid) === Number(data.privileges.uid)) || data.privileges.isAdminOrMod;
      const editButtonHtml = isAuthorized ? `<a href="#!" data-place-author="${m.uid}" data-topic="${m.tid}" class="btn circle-button edit-place card-link"><i class="fa-regular fa-pen-to-square"></i></a>` : '';
      ...
      const author = data.posts[0].user
      ...
      <div id="metaTab" class="tab-content" data-authorcolor="${authorColor}" data-authoravatar="${authorAvatar}" data-authorname="${author.displayname || author.username}" data-authorletter="${authorLetter}">
        ...
        <div id="descTab" class="tab-content mb-5">
          <div class="tab-pane fade mb-3 active show" id="navDescription" role="tabpanel" aria-labelledby="navDescription-tab" tabindex="0">
           <div class="white-space-pre-line">${data.posts[0].content}</div>
          </div>
      ...
      data.posts[0].content = ` ${gallerySwiper}${topicMapDiv}${modifiedContent}`;
    }

  } catch (error) {
    //data.topic.unimap_debug = data.privileges.isAdminOrMod ? error.message : 'Error occurred...';
  }
