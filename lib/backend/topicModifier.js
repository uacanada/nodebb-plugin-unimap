...
module.exports = (data, settings, winston) => {
  ...
  try {
    ...
    if (mapFields?.placeTitle) {
      ...
      const topicEventDiv = m.eventStartDate ? `<div id="topicEvent">
      ...
      <div id="metaTab" class="tab-content" data-authorcolor="${authorColor}" data-authoravatar="${authorAvatar}" data-authorname="${author.displayname || author.username}" data-authorletter="${authorLetter}">
        ...
        <div class="place-ctx-buttons p-3">
          ...
          <a href="#!" data-place-author="${m.uid}" data-topic="${m.tid}" class="d-none btn circle-button edit-place card-link p-4"><i class="fa-regular fa-pen-to-square"></i></a>
    ...
    winston.warn(`[UniMap Error] topicModifier: ${error.message}`);
  }
