...
module.exports.reImportPlaces = async function(req, res, helpers) {
    const body = req.body;
    if (!body.newPlaces || !req.uid) {
        helpers.formatApiResponse(200, res, { error: "newPlaces are not valid!", details: {} });
        return;
    }
    winston.verbose(body.newPlaces)
    const placesJson = JSON.parse(body.newPlaces);

    
    // await db.setObjectField(DATABASE_KEYNAME+':places', placesJson);
    helpers.formatApiResponse(200, res, { status: "success", placesJson });
}
