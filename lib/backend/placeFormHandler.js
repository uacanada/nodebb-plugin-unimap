...
module.exports.reImportPlaces = async function(req, res, helpers) {
    ...
    if (!Array.isArray(newPlaces) || !newPlaces.every(isValidPlace)) {
        helpers.formatApiResponse(200, res, { error: "Provided newPlaces data is not valid!", details: {} });
        return;
    }
...
module.exports.handleAddPlaceRequest = async function (req, res, helpers) {
    ...
    try {
        ...
        // Sanitize and validate user input for the new location.
        // NOTE: If you introduce new fields, ensure their inclusion in the 'fields' object
        // by defining corresponding sanitization logic within the 'lib/backend/inputValidator' module.
        ...
        let assignedAuthor = { uid: req.uid }
        const isAdmin = await privileges.users.isAdministrator(req.uid);
    
        if (isAdmin && fields.assignedAuthorUid) { 
            assignedAuthor.uid = Number(fields.assignedAuthorUid); 
        }
       
        const userData = await User.getUserFields(assignedAuthor.uid, ['username', 'userslug', 'picture', 'fullname','groupTitle'])
        assignedAuthor.username = userData.username;
        assignedAuthor.userslug = userData.userslug;
        assignedAuthor.picture = userData.picture;
        assignedAuthor.fullname = userData.fullname;
        assignedAuthor.groupTitle = userData.groupTitle;
        ...
        fields.assignedAuthor = assignedAuthor;
